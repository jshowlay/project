...
async function setupTrendsView() {
  const client = await pool.connect();
  try {
    console.log('Setting up trends materialized view...');
    ...
    console.error('❌ Error setting up trends view:', error);
    throw error;
  } finally {
    client.release();
  }
...
async function main() {
  try {
    await setupTrendsView();
    console.log('🎉 Trends view setup completed successfully!');
  } catch (error) {
